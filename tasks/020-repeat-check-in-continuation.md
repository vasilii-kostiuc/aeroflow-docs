...
Выполнено при закрытии задачи:

* `php bin/phpunit tests/Unit tests/Application` в core (152 теста) и
  `php bin/phpunit` в playback (80 тестов) — зелёные.
* Сквозной прогон через реальные контейнеры (не unit-стаб) с интервалом в 1
  минуту, минуя core (публикация `RequestAnnouncementPlayback`/`StopAnnouncementRepeat`
  напрямую в RabbitMQ): job родился `Scheduled`; delayed wake (реальный
  AMQP-транспорт `scheduler`, dead-letter delay exchange) разбудил job через
  минуту; ручное завершение тика (`playback:job:complete`) корректно
  перевзвело job в `Scheduled` с новым `scheduledAt` и заново запланировало
  wake; `stop_repeat`, отправленный между тиками, немедленно терминировал job
  и передал в core ровно один `completed`-receipt; повторная доставка
  `stop_repeat` не продублировала receipt; осиротевший wake для уже
  терминального job корректно no-op'нулся без ошибок.
* При этом прогоне найден и исправлен инфраструктурный баг, из-за которого
  повтор был бы полностью нерабочим в реальном деплое: `docker-compose.yml`
  playback явно перечислял транспорты, которые слушает воркер
  (`messenger:consume async agent_events`), и не включал новый `scheduler` —
  сообщения `WakeScheduledPlaybackJob` публиковались, но их никто не
  потреблял, job навсегда оставался бы в `Scheduled` после первого тика.
  Исправлено (добавлен `scheduler` в команду воркера и переменная
  `MESSENGER_SCHEDULER_TRANSPORT_DSN` в оба `environment:`-блока, по образцу
  остальных DSN). Unit/application-тесты этого не ловят, так как in-memory
  транспорт в тестах не моделирует реальный список потребляемых очередей.
* Известное принятое ограничение (не исправлено этим срезом, зафиксировано в
  `domain-model.md`): re-arm между тиками не публикует событие в core, поэтому
  read-модель `ListPlaybackQueue` показывает continuation играющей непрерывно
  всё время открытой регистрации, а не только в момент реального тика.
  Расхождение видно только на экране «Статус» и не влияет на сам домен
  (`PlaybackJob`/`Announcement` состояние корректно на всех уровнях).
* Найденный при ревью узкий edge case, не исправлен (вне файлов, затронутых этой
  задачей): `FlightAnnouncementConfig::assertRepeatRule` допускает enabled
  continuation-конфиг с языками, но без `repeatEveryMinutes` (поле `?int`,
  проверяется только обратная импликация). До 020 это было безобидно
  (continuation нигде не повторялся); теперь такой конфиг создаст continuation
  `Announcement`, который сыграет один раз вместо серии, а `endContinuationRepeat`
  на закрытии всё равно найдёт его и молча отправит `StopAnnouncementRepeat` в
  playback (там — идемпотентный no-op, т.к. job не повторяемый). Не крашится, не
  портит состояние — деградирует до одноразового звучания. Требует отдельного
  решения (сделать `repeatEveryMinutes` обязательным для continuation в
  `FlightAnnouncementConfig`, либо игнорировать) в контексте задачи,
  затрагивающей `FlightAnnouncementConfig`.

## Documentation Updates

Выполнено:

* `domain-model.md` — раздел `PlaybackJob`: «Пятый срез» (рождение в
  `Scheduled`, `Playing -> Scheduled` re-arm, `repeatCancelled`,
  delayed-пробуждение через выделенный transport `scheduler`, известное
  ограничение read-модели); раздел `Announcement` — `repeatEveryMinutes`,
  `repeatEndedAt`, `endRepeat()`/`AnnouncementRepeatEnded`, контракт
  `StopAnnouncementRepeat`; `FlightOccurrence` — `continueCheckIn()`/
  `CheckInContinued` помечены `@deprecated`; переименование
  `AnnouncementLauncherInterface` → `OccurrenceAnnouncementPort`; открытые
  решения — переход от per-tick `Announcement` к playback-owned repeat закрывает
  прежний открытый вопрос «правила повторов continuation».
* `architecture.md` — Core → Playback: четвёртая команда на
  `announcement_playback`; Playback → Core: без нового типа события (re-arm
  ничего не публикует, термация переиспользует `Completed`).
* `README.md` — абзац о реализованных срезах очереди дополнен описанием
  playback-owned повтора рядом с 018/019.
* `adr/002-inbound-message-discrimination.md` — четвёртый тип на одной очереди
  тем же `match`; прогноз уровня 3 («потолок — четыре, две уходят на
  emergency») скорректирован по факту (четыре неэкстренных типа уже на одной
  очереди).
* `aeroflow-web/architecture.md` — без изменений: фронтенд вне скоупа задачи
  (серия управляется автоматически), существующая пометка «`check_in_continuation`
  не входит в ручной поток» остаётся верной.
